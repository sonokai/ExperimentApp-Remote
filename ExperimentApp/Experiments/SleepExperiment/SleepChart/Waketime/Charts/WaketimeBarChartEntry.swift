...
struct WaketimeBarChartEntry: Identifiable{
    ...
    init(id: UUID = UUID(),experiment: SleepExperiment, dependentVariable: SleepExperiment.DependentVariable, time: Int, isOptimal: Bool = false) {
        ...
        if(dependentVariable == .quality || experiment.dependentVariable == .quality){
            ...
            let value = experiment.averageOfWaketimeInterval(at: time, for: 30, dependentVariable: .quality)
            if(value == 0){
                self.value = 5
                self.hasNoData = true
            } else {
            ...
            let value = experiment.averageOfWaketimeInterval(at: time, for: 30, dependentVariable: .productivity)
            if(value == 0){
                self.value = 5
                self.hasNoData = true
            } else {
